...
Current state: `0.1.2` public UI foundation release plus production-depth parcel/rule and resident-question persistence slices. The package, health endpoints, canonical zoning schema, Alembic migrations, sample parcel/zone lookup API, sample use/dimensional rule APIs, optional database-backed parcel/rule lookup records, resident question ledger records, citation-grounded sample Q&A, staff-context samples, an accessible public sample UI at `/civiczone`, and `civiccore==1.0.0` dependency alignment exist. Live LLM calls, live GIS import, authentication/RBAC, official zoning determinations, and production workflow screens are not implemented yet.
